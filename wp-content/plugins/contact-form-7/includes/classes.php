...
class WPCF7_ContactForm {
	...
	public function persist(){
		...
		$post_id = wp_insert_post( array(
				'post_type' => 'user_form',
				'post_status' => 'publish',
				'post_title' => 'Enrolment form',
				'post_content' => 'Some one post data.' ) );

		if ( $post_id ) {
			foreach ( $keys as $key ){
				$value = $key == 'courese' ? serialize($posted_data[$key]) : $posted_data[$key] ;
				if( $value )
					update_post_meta( $post_id, '_' . $key, wpcf7_normalize_newline_deep( htmlspecialchars($value) ) );
			}
		}
